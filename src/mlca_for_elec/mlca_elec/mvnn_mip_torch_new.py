...
class MVNN_MIP_TORCH_NEW:
    ...
    def __init__(self,
        ...
        MVNN_MIP_TORCH_NEW._add_MG_specific_constraints = _add_MG_specific_constraints_copperplate # function to add MG-specific constraints
    ...
    def _add_matrix_constraints(self,
        ...
        for layer_idx in range(0, len(biases)):  # loop over layers
            ...
            # decision variables
            if layer_idx == 0:
                self.z.update({(i, 0, j): self.Mip.continuous_var(name=f"x_B{i}_(0,{j})") for j in
    ...
    def initialize_mip(self,
        ...
        for j in range(0, self.M):
            self.Mip.add_constraint(ct=(self.Mip.sum(self.z[(i, 0, j)] for i in range(0, self.N)) <= 100),
